erpnext.PointOfSale.Controller = class {
	...
	prepare_menu() {
		...
		this.page.add_menu_item(__("Print Receipt [shift ctrl 1]"), this.print_last_receipt.bind(this), false, "Shift+Ctrl+1");

		this.page.add_menu_item(__("Email Receipt [shift ctrl 2]"), this.email_last_receipt.bind(this), false, "Shift+Ctrl+2");

		this.page.add_menu_item(__("New Order [shift ctrl 3]"), this.new_order_shortcut.bind(this), false, "Shift+Ctrl+3");
	...
	save_draft_invoice() {
		console.log("Attempting to draft invoice")
	...
	raise_customer_selection_alert() {
		frappe.dom.unfreeze();
		// Try to set default customer 'Consumidor Final' if not already set
		if (!this.frm.doc.customer) {
			frappe.db.get_value('Customer', { customer_name: 'Consumidor Final' }, 'name').then(res => {
				if (res && res.message && res.message.name) {
					this.frm.set_value('customer', res.message.name).then(() => {
						frappe.show_alert({
							message: __("Default customer 'Consumidor Final' applied."),
							indicator: "green",
						});
					});
				}
